...
<documents>
  ...
  <document path="docs/snapshot-fixtures/architecture.md">
4: # Architecture
5: 
6: ## 3.2 Payment Integration
7: 
8: The payment service processes transactions in batch via the /v2/batch endpoint.
9: All payment operations MUST use batch mode to stay within rate limits.
  </document>

  <document path="docs/snapshot-fixtures/project-context.md">
1: # Project Context
2: 
3: ## Technology Stack & Versions
4: 
5: - TypeScript ^5.7.2
6: - Node.js / Edge (Vercel)
7: 
8: ## Async Patterns
9: 
10: Use async/await — no raw .then() chains.
  </document>

</documents>

<diff>
diff --git a/src/payments/handler.ts b/src/payments/handler.ts
index 1111111..2222222 100644
--- a/src/payments/handler.ts
+++ b/src/payments/handler.ts
@@ -10,3 +10,5 @@
-await paymentClient.batch(transactions)
+for (const tx of transactions) {
+  await paymentClient.process(tx)
+}
diff --git a/src/notifications/email-sender.ts b/src/notifications/email-sender.ts
...
<pr_metadata>
  <title>Snapshot parity canonical fixture</title>
  <repo>delfini-fixtures/snapshot-parity</repo>
  <pr_number>42</pr_number>
  <head_sha>0000000000000000000000000000000000000001</head_sha>
  <base_sha>0000000000000000000000000000000000000002</base_sha>
  <changed_file_count>2</changed_file_count>
</pr_metadata>
